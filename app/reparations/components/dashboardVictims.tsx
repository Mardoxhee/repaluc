    ...
    <div className="w-full px-6 py-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-5 gap-6 mb-8">
        <div className="bg-white rounded-2xl shadow p-6 flex flex-col items-center justify-center border border-gray-100">
          ...
          <span className="text-xs text-gray-500 mt-1">Victimes enregistrées</span>
        </div>
        <div className="bg-white rounded-2xl shadow p-6 flex flex-col items-center justify-center border border-gray-100">
          <FiLayers className="text-purple-400 text-3xl mb-2" />
          <span className="text-3xl font-bold text-gray-900">{victimsByCat.reduce((a, b) => a + b.value, 0)}</span>
          <span className="text-xs text-gray-500 mt-1">Catégories</span>
        </div>
        <div className="bg-white rounded-2xl shadow p-6 flex flex-col items-center justify-center border border-gray-100">
          <FiGrid className="text-blue-400 text-3xl mb-2" />
          <span className="text-3xl font-bold text-gray-900">{victimsByProg.reduce((a, b) => a + b.value, 0)}</span>
          <span className="text-xs text-gray-500 mt-1">Programmes</span>
        </div>
        <div className="bg-white rounded-2xl shadow p-6 flex flex-col items-center justify-center border border-gray-100">
          <FiAward className="text-yellow-400 text-3xl mb-2" />
          <span className="text-3xl font-bold text-gray-900">{victimsByPrej.reduce((a, b) => a + b.value, 0)}</span>
          <span className="text-xs text-gray-500 mt-1">Préjudices</span>
        </div>
        <div className="bg-white rounded-2xl shadow p-6 flex flex-col items-center justify-center border border-gray-100">
          <span className="inline-block bg-green-100 text-green-700 rounded-full px-2 py-1 text-xl mb-2">✔</span>
          <span className="text-3xl font-bold text-gray-900">{totalPrisEnCharge}</span>
          <span className="text-xs text-gray-500 mt-1">Prises en charge</span>
        </div>
      </div>
      {/* Graphiques */}
